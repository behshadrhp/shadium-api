...
class CommentAdmin(ExportActionMixin, admin.ModelAdmin):
    """
    Monitoring All user Comments.
    """
    
    list_display = ["user", "post", "is_deleted", "created_at", "updated_at"]
    list_filter = ["user", "post", "is_deleted", "created_at", "updated_at"]
    search_fields = ["user__email__icontains", "post__title__icontains", "message__icontains"]
    fields = [ "user", "post", "replay", "message", "is_deleted", "created_at", "updated_at"]
